...
dataset._df = dataset._df.loc[valid_Tcs].reset_index(drop=True)
assert len(valid_Tcs)==len(dataset._df)
...
duplicates = dataset._df.duplicated(subset=['comps_pymatgen'], keep=False)
print(f"duplicates.value_counts():{duplicates.value_counts()}")
dataset._df = dataset._df.drop_duplicates(subset=['comps_pymatgen'], keep=False).reset_index(drop=True)
print(f"len(dataset.dataframe):{len(dataset._df)}")

# save csv
dataset._df[["name", "Tc"]].to_csv("preprocessed_supercon.csv")