...
def distance_matrix(comps:list[Composition] | list[float] |list[list[float]],
                    metric:str, elemental_set:list[str]|None = None,
                    exclude_expanded_elements:bool=True):
    """simple wrapper of cdist from scipy.spatial.distance
    ...
    Args:
        ...
        - metric: str - mapping aliases,
            ...
            - "l_infty" | "maximum" | "tchebychev": to "chebyshev"
                - \max_i(|x_i-y_i|)
            - any other valid metric for cdist(metric).
    ...
    elif all(isinstance(frac, float) for frac in comps):
        comps=[comps]
    ...
    alias_map = alias_map | {al: "euclidean" for al in ("l2", "L1")}
    metric = alias_map.get(metric, metric)
    return cdist(comps, comps, metric=metric)
...
def make_duplicates_group(index, d1_dist_matrix, dinfty_dist_matrix, elements_sets, linfty_cutoff=0.01, l1_cutoff=0.02, cross_elements_set=False):
